...
class Worker(Thread):
    def __init__(self, device, client: cc_lib.client.Client, zway: Zway):
                ...
                if time.time() - command.timestamp <= config.Controller.max_command_age:
                    ...
                    try:
                        if command.message.data:
                            data = self.__device.getService(command.service_uri, self.__zway, **json.loads(command.message.data))
                        else:
                            data = self.__device.getService(command.service_uri, self.__zway)
...
class Controller(Thread):
    def __init__(self, device_manager: DeviceManager, client: cc_lib.client.Client, zway: Zway):
            ...
            try:
                ...
                try:
                    ...
                    if not device.id in self.__worker_pool:
                        worker = Worker(device, self.__client, self.__zway)
                        worker.start()
